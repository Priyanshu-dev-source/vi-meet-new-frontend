...
//             );
//             const finalServers = [...credentials];
//             //  hasStun
//             //   ? credentials
//             //   : [{ urls: ["stun:stun.l.google.com:19302", "stun:stun1.l.google.com:19302"] }, 
//             //   ...credentials];
...
const JoinMeetPage = ({
  ...
  const [remoteStreams, setRemoteStreams] = useState({}); // { [socketId]: MediaStream }
  const remoteStreamsRef = useRef({});
  // ICE servers stored in a ref so createPeerConnection always reads the latest value,
  // even when called from inside the socket effect closure (which captures an early snapshot).
  const iceServersRef = useRef([]);
  const [iceServersReady, setIceServersReady] = useState(false);
  const sendersRef = useRef({}); // { [peerId]: { audio: RTCRtpSender|null, video: RTCRtpSender|null } }
  // Always read from ref — never stale inside closures
  const getRtcConfig = () => ({
    bundlePolicy: 'max-bundle',
    iceCandidatePoolSize: 0, // 0 until servers are confirmed loaded
    ...
    const fetchTurnCredentials = async () => {
      try {
        ...
        if (response.ok) {
          const credentials = await response.json();
          if (Array.isArray(credentials) && credentials.length > 0) {
            // Write into the ref — all future createPeerConnection calls will see these servers
      ...
      // Signal that ICE servers are ready — peer connections can now be safely created
  ...
  const createPeerConnection = (peerId) => {
    if (peerConnectionsRef.current[peerId]) return peerConnectionsRef.current[peerId];
    // getRtcConfig() reads from iceServersRef.current — always up-to-date, never stale
      ...
      newSocket.on("connect", async () => {
        ...
        if (dynamicRoute === "new") {
          newSocket.emit("create-room", async (roomId) => {
            ...
            // Also join the socket room so WebRTC signaling events are received
  ...
  useEffect(() => {
    onValue(usersRef, (snapshot) => {
      ...
      if (usersData && usersData.users) {
        ...
        // setup peer connections for new users — only after ICE servers are loaded
        const peerIds = Object.keys(usersData.users);
        if (iceServersReady) {
          peerIds.forEach((peerId) => {
            if (socket && peerId !== socket.id && !peerConnectionsRef.current[peerId]) {
